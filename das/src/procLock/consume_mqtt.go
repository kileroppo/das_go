...
var msgCallback mqtt.MessageHandler = func(client mqtt.Client, msg mqtt.Message) {
	...
	rabbitmq.SendGraylogByMQ("王力IoT-mqtt->DAS: dev[%s], cmd[%d], seqId[%d]; Mqtt-Topic: %s, strHexMsg: %s", wlMsg.DevId.Uuid, wlMsg.Cmd, wlMsg.SeqId, msg.Topic(), hex.EncodeToString(msg.Payload()))
