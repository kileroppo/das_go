...
func (p *Serload) ProcessJob() error {
	...
	if !strings.ContainsAny(p.DValue, "{ & }") { // 判断数据中是否包含{ }，不存在，则是加密数据
		...
		log.Info("[", p.Imei, "] ApiVersion: ", myHead.ApiVersion, ", ServiceType: ", myHead.ServiceType, ", MsgLen: ", myHead.MsgLen, ", CheckSum: ", myHead.CheckSum)
		...
		if checkSum != myHead.CheckSum {
			log.Error("[", p.Imei, "] ProcessJob() CheckSum failed, src:", myHead.CheckSum, ", dst: ", checkSum)
		...
		if constant.SERVICE_TYPE_UNENCRY == myHead.ServiceType { // 不加密
			...
			var err_aes error
			p.DValue, err_aes = util.ECBDecrypt(strData, myKey)
			if nil != err_aes {
				log.Error("[", p.Imei, "] util.ECBDecrypt failed, strData:", strData, ", key: ", myKey, ", error: ", err_aes)
				return err_aes
			}
			log.Info("[", p.Imei, "] After ECBDecrypt, data.Msg.Value: ", p.DValue)
		}
	}

	p.DValue = strings.Replace(p.DValue, "#", ",", -1)
	log.Debug("[", p.Imei, "] ProcessJob() p.DValue after: ", p.DValue)
	...
	case constant.Add_dev_user_step: // 新增用户步骤
		{
			...
			if err_step := json.Unmarshal([]byte(p.DValue), &addUserStep); err_step != nil {
	...
	case constant.Upload_lock_active: // 锁激活状态上报
		{
			...
			if err_lockActive := json.Unmarshal([]byte(p.DValue), &lockActive); err_lockActive != nil {
	...
	case constant.Get_Upgrade_FileInfo: // 锁查询升级固件包信息
		{
			...
			if err := json.Unmarshal([]byte(p.DValue), &upQuery); err != nil {
	...
	case constant.Download_Upgrade_File: // 锁下载固件升级包（锁—>后台，分包传输）
		{
			...
			if err := json.Unmarshal([]byte(p.DValue), &upReq); err != nil {
