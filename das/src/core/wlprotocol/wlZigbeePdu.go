...
func (pdu *AddDevUserZigbee) Encode(uuid string) ([]byte, error) {
...
func (pdu *AddDevUserZigbee) Decode(bBody []byte, uuid string) error { // 包体解包，AES解密
	return nil
}

//8. Wifi设置(0x37)(服务器-->前板)
// Ssid（32）+密码（32）
func (pdu *WiFiSetZigbeeSsid) Encode(uuid string) ([]byte, error) {
	buf := new(bytes.Buffer) // 定义一个buffer，给了打包数据使用

	// 组body
	var err error
	if err = binary.Write(buf, binary.BigEndian, pdu.DType); err != nil {
		log.Error("binary.Write failed:", err)
		return nil, err
	}
	...
	toDevice_byte := buf.Bytes()
	log.Debug("[ ", uuid, " ] WiFiSetZigbeeSsid Encode [ ", hex.EncodeToString(toDevice_byte), " ]")
...
func (pdu *WiFiSetZigbeeSsid) Decode(bBody []byte, uuid string) error {
	var err error
	var DValue []byte
	...
	log.Debug("[ ", uuid, " ] WiFiSetZigbeeSsid Decode [ ", hex.EncodeToString(DValue), " ]")

	//3. 解包体
	buf := bytes.NewBuffer(DValue)
	if err = binary.Read(buf, binary.BigEndian, &pdu.DType); err != nil {
...
func (pdu *WiFiSetZigbeePwd) Encode(uuid string) ([]byte, error) {
	buf := new(bytes.Buffer) // 定义一个buffer，给了打包数据使用
	...
	toDevice_byte := buf.Bytes()
	log.Debug("[ ", uuid, " ] WiFiSetZigbeePwd Encode [ ", hex.EncodeToString(toDevice_byte), " ]")
...
func (pdu *WiFiSetZigbeePwd) Decode(bBody []byte, uuid string) error {
	var err error
	var DValue []byte
	...
	log.Debug("[ ", uuid, " ] WiFiSetZigbeePwd Decode [ ", hex.EncodeToString(DValue), " ]")
